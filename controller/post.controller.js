...
export const createPost = async (req , res )=>{
       try {
        ...
        if(!text && !img){
            return res.status(400).json({err : "post must have text or img"})
        }
...
export const createComment = async ( req , res )=>{
    try {
        ...
        if(!text){
            return res.status(401).json({ error : " no text"})
        }

        const post = await Post.findOne({_id : postId}) 

        if(!post){
            return res.status(401).json({ error : "page not foundt"})
        }
        ...
        console.log(`error in the createComment controller ${error}`);
        res.status(500).json({ err : "internal server error"})
    }
}
